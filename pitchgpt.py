...
class PitchGPT:
    ...
    def analyze_pitch(self, pitch_text, prompt_key='default_analysis', custom_prompt=None):
        ...
        # 3) ensure the template can accept {context}
        if "{context}" not in prompt_template:
            # append a simple context block if the current template lacks {context}
            prompt_template = f"{prompt_template}\n\nContext:\n{{context}}"

        prompt = prompt_template.format(pitch_text=pitch_text, context=context)
